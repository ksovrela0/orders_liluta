    ...
    case 'get_list_proccess':
        ...
        if($path_id == 2){
            ...
            $db->setQuery("SELECT 		cut_glass.id,
                                        ...
                                        CONCAT('<span class=\"',IF(DATEDIFF(orders.datetime_finish,CURDATE()) > 3 AND glass_status.id IN (3),'','make_me_red '),'status_',glass_status.id,'\">',glass_status.name,'</span>') AS status,
                                        CASE
            ...
            $db->setQuery(" SELECT  orders_product.id,
                                    ...
                                    CASE
                                        ...
                                        WHEN orders_product.status_id = 4 THEN CONCAT('<span class=\"',IF(DATEDIFF(orders.datetime_finish,CURDATE()) > 3 AND glass_status.id IN (3),'','make_me_red '),'status_',glass_status.id,'\">',glass_status.name,'</span>')
            ...
            $db->setQuery(" SELECT * FROM (SELECT	products_glasses.id,
                                    ...
                                    glass_colors.name AS color,
                                    CONCAT('<b class=\"',IF(DATEDIFF(orders.datetime_finish,CURDATE()) > 3 AND glass_status.id IN (3),'','make_me_red '),'\">',products_glasses.glass_width,'</b> X <b>', products_glasses.glass_height,'</b> მმ') AS param,
