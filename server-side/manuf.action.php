...
switch ($act){
    ...
    case 'get_list':
		...
		$cols[]      =      $_REQUEST['cols'];
            ...
            $db->setQuery(" SELECT  glass_manuf.id,
                                    glass_manuf.name,
                                    glass_manuf.info
                            FROM    glass_manuf
                            WHERE   glass_manuf.actived = 1");
    ...
    case 'save_manuf':
        $id             = $_REQUEST['id'];
        $manuf_name      = $_REQUEST['manuf_name'];
        $manuf_info     = $_REQUEST['manuf_info'];
        ...
        if($id == ''){
            ...
            $db->setQuery(" UPDATE  glass_manuf 
                            SET     `name` = '$manuf_name',
                                    `info` = '$manuf_info'
                            WHERE   id = '$id'");
            $db->execQuery();
    ...
    <fieldset class="fieldset">
        ...
        <div class="row">
            <div class="col-sm-6">
                ...
                <input value="'.$res['name'].'" data-nec="0" style="height: 18px; width: 95%;" type="text" id="manuf_name" class="idle" autocomplete="off">
            </div>
            <div class="col-sm-6">
                <label>დამატებითი ინფორმაცია</label>
                <input value="'.$res['info'].'" data-nec="0" style="height: 18px; width: 95%;" type="text" id="manuf_info" class="idle" autocomplete="off">
            </div>

            
        </div>
    </fieldset>
    <input type="hidden" id="manuf_id" value="'.$res[id].'">
    ';
