    ...
    $data = '   <fieldset class="fieldset">
                    <legend>აირჩიეთ ხარვეზიანი მინები</legend>
                        <div class="row">';
                            foreach($glass_ids AS $glass){
                                $data .= '  <div class="col-sm-4" style="text-align: center;">
                                                <label>მინა #'.$glass['glass_id'].' '.$glass['si'].' <input type="checkbox" class="glass_error" data-id="'.$glass['id'].'"></label>
                                                <input type="tel" min="1" class="glass_pyramids" placeholder="პირამიდის #" data-id="'.$glass['id'].'">
                                ...
                                foreach($atxods AS $at){
                                    $data .= '  <div class="col-sm-4" style="text-align: center;">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-sm-6"><input style="width:99%;" type="text" class="atxod_height" data-id="'.$at['id'].'" value="'.$at['height'].'"></div>
                                                    </div>
                                                </div>';
                                }
                            $data .= '</div>
...
switch ($act){
    ...
    case 'finish_glass_proc':
        ...
        if($proc_id == 2){
            ...
            $db->setQuery(" SELECT  COUNT(*) AS cc
                            FROM    orders_product
                            WHERE   order_id = '$order_id' AND orders_product.actived = 1 AND status_id NOT IN (3)");
...

			{
...

                {
                    ...
                    elseif($columns[$j] == "butil" OR $columns[$j] == "glasses_grouped_cc"){
						...
						$g = array('field'=>$columns[$j],'hidden' => $hidebutil, 'encoded'=>false,'title'=>$columnNames[0][$a],'filterable'=>array('multi'=>true,'search' => true), 'width' => 40);
                    ...
                    elseif($columns[$j] == "lameks"){
						...
						$g = array('field'=>$columns[$j],'hidden' => $hidefiri,'encoded'=>false,'title'=>$columnNames[0][$a],'filterable'=>array('multi'=>true,'search' => true), 'width' => 50);
                    ...
                    elseif($columns[$j] == "total_to_pay" OR $columns[$j] == "avans" OR $columns[$j] == "add_money" OR $columns[$j] == "left_to_pay" OR $columns[$j] == "price_glass_proc"){
                        $g = array('field'=>$columns[$j],'hidden' => $cexis_ufrosi,'encoded'=>false,'title'=>$columnNames[0][$a],'filterable'=>array('multi'=>true,'search' => true));
                    }
                    else{
    ...
    case 'get_list_proccess':
        ...
        if($path_id == 2){
            $db->setQuery("SELECT 		cut_glass.id,
                                        ...
                                        GROUP_CONCAT(CONCAT('ID: ', products_glasses.id, ' ზომები: ', products_glasses.glass_width, 'მმ X ',products_glasses.glass_height, 'მმ - ',glass_st.name,' - <span data-id=\"',products_glasses.id,'\" class=\"print_shtrixkod\"><img style=\"width:40px\" src=\"assets/img/print.png\"></span>') SEPARATOR ', <br>') AS glasses,
            ...
            $db->setQuery(" SELECT  orders_product.id,
                                    ...
                                    GROUP_CONCAT(DISTINCT CONCAT('№-',products_glasses.id,' ',glass_options.name, ' - ', products_glasses.glass_width,'მმX',products_glasses.glass_height, 'მმ პირამიდა: ', IFNULL(IF(IFNULL((SELECT path_group_id FROM glasses_paths WHERE status_id IN (1,2,4,5) AND glass_id = products_glasses.id AND actived = 1 LIMIT 1), 0) != glasses_paths.path_group_id, glasses_paths.pyramid,(SELECT path_2.pyramid FROM glasses_paths AS path_2 WHERE path_2.glass_id = products_glasses.id AND path_2.sort_n = glasses_paths.sort_n - 1 AND actived = 1)),''),' <span data-id=\"',products_glasses.id,'\" class=\"print_shtrixkod\"><img style=\"width:40px\" src=\"assets/img/print.png\"></span>') SEPARATOR ',<br>') AS glasses,
    ...
    border-radius: 5px;\">რიგში</span>'
            ...
            $db->setQuery(" SELECT	products_glasses.id,
                                    CONCAT(glass_options.name,' <span data-id=\"',products_glasses.id,'\" class=\"print_shtrixkod\"><img style=\"width:40px\" src=\"assets/img/print.png\"></span>') AS option,
                                    glass_type.name AS type,
                                    glass_colors.name AS color,
                                    CONCAT(products_glasses.glass_width,'მმX', products_glasses.glass_height,'მმ') AS param,
                                ...
                                END AS pyramid,
                                    ...
                                    WHEN glass_status.id = 1 THEN IF((SELECT path_group_id FROM glasses_paths WHERE status_id IN (1,2,4,5) AND glass_id = products_glasses.id AND actived = 1 LIMIT 1) != glasses_paths.path_group_id, '<span style=\"padding: 5px;
    color: white;
    background: radial-gradient(#1448ce 0.3%, #5e28ee 90%);
