...
switch ($act){
                    ...
                    if($proc_id == 5 AND !in_array($user_gr, array(14,13,12,5,1))){
                        ...
                        if($proc_id == 3){
                            ...
                            if($cc >= 30){
                                ...
                                $db->setQuery("UPDATE products_glasses SET kalioni_group='$kalioni_group', kalioni_group_user_id = '$user_id' WHERE id = '$glass_id'");
                                $db->execQuery();
                            }
                        }
    ...
    case 'start_few':
        ...
        if(count($err_glass_ids) == 0){
            ...
            foreach($ok_codes AS $code){
                ...
                $db->setQuery("UPDATE products_glasses SET kalioni_group='$kalioni_group', kalioni_group_user_id = '$user_id' WHERE id = '$code'");
                $db->execQuery();
    ...
    case 'get_list_kalioni_group':
        ...
        $data = $result;
        break;
    case 'get_list_kalioni_group_ne':
        $columnCount = 		$_REQUEST['count'];
        $cols[]      =      $_REQUEST['cols'];

        $db->setQuery("SELECT	products_glasses.kalioni_group,GROUP_CONCAT(CONCAT('<div style=\"display:flex;\">ID: ',products_glasses.id, ' ზომები: ', CONCAT(glass_options.name,' ', IFNULL((SELECT CONCAT('(',products.name,')') FROM orders_product JOIN products ON products.id = orders_product.product_id AND products.id IN (2,3) WHERE orders_product.id = products_glasses.order_product_id),'')),CONCAT('<b class=\"',IF(DATEDIFF(orders.datetime_finish,CURDATE()) > 3 OR glass_status.id IN (3),'','make_me_red '),'\">',products_glasses.glass_width,'</b> X <b>', products_glasses.glass_height,'</b> მმ '), glass_type.name,' ', glass_colors.name,' - პირ: ',products_glasses.last_pyramid,IF(glasses_paths.status_id = 4,' - <span class=\"status_1\">დახარვეზებული</span> ',''), '  <span data-id=\"',products_glasses.id,'\" style=\"','\" class=\"print_shtrixkod\"><img style=\"width:20px\" src=\"assets/img/print.png\"></div>'
        ...
        $data = $result;
        break;
    case 'get_list_kalioni_group_mine':
        $columnCount = 		$_REQUEST['count'];
        $cols[]      =      $_REQUEST['cols'];
...

        {
...

			{
...

                {
                    ...
                    else if($columns[$j] == "product_glasses_all"){
                        $g = array('field'=>$columns[$j],'encoded'=>false,'title'=>$columnNames[0][$a],'filterable'=>array('multi'=>true,'search' => true, 'cell' => array('operator'=>'contains','suggestionOperator'=>'contains')), 'width' => 1050);
                    }
                    else if($columns[$j] == "gr_id"){
                        $g = array('field'=>$columns[$j],'encoded'=>false,'title'=>$columnNames[0][$a],'filterable'=>array('multi'=>true,'search' => true, 'cell' => array('operator'=>'contains','suggestionOperator'=>'contains')), 'width' => 40);
                    }
                    else{
        ...
        if($path_id == 2){
            ...
            $db->setQuery(" SELECT  orders_product.id,
                                    ...
                                    CASE
                                        ...
                                        WHEN orders_product.status_id = 4 THEN CONCAT('<span class=\"',IF(DATEDIFF(orders.datetime_finish,CURDATE()) > 3 OR glass_status.id IN (3),'','make_me_red '),'status_',glass_status.id,'\">',glass_status.name,'</span>')
                                        ELSE CONCAT('<span class=\"',IF(DATEDIFF(orders.datetime_finish,CURDATE()) > 3,'','make_me_red '),'\" style=\"padding: 5px;color: white;background: radial-gradient(#1448ce 0.3%, #5e28ee 90%);border-radius: 5px;\">რიგში</span>')
                                    END AS glasses2,
    ...
    case 'get_list_product':
        ...
        GROUP_CONCAT(CONCAT('№-',products_glasses.id,' ',glass_options.name, '(',glass_manuf.name,') <span glass-id=\"',products_glasses.id,'\" prod-id=\"',orders_product.id,'\" class=\"change_sizes\"><b>',products_glasses.glass_width,'</b> X <b>', products_glasses.glass_height,'</b></span> მმ ',glass_colors.name,' - <span class=\"status_',glass_status.id,'\">',glass_status.name,'</span> ', IF((SELECT COUNT(*) FROM glasses_paths WHERE actived = 1 AND glass_id = products_glasses.id AND status_id IN (1,2,4,5,6)) = 0,'<span class=\"status_finished\">დასრულებული</span>',CASE
                            ...
                            ELSE IF(products_glasses.go_to_cut != 1,IFNULL(IFNULL((SELECT name FROM `groups` WHERE id = (SELECT IF(gp1.status_id = 3 OR gp1.status_id IS NULL,gp2.path_group_id,gp1.path_group_id) FROM glasses_paths AS gp2 LEFT JOIN glasses_paths AS gp1 ON gp1.glass_id = gp2.glass_id AND gp1.sort_n = gp2.sort_n-1 AND gp1.actived=1 WHERE gp2.status_id IN (1,2) AND gp2.glass_id = products_glasses.id AND gp2.actived = 1 ORDER BY gp1.sort_n ASC LIMIT 1)), IFNULL((SELECT name FROM `groups` WHERE id = (SELECT path_group_id FROM glasses_paths WHERE status_id IN (4,5) AND actived = 1 AND glass_id = products_glasses.id ORDER BY sort_n ASC LIMIT 1)), (SELECT name FROM `groups` WHERE id = (SELECT path_group_id FROM glasses_paths WHERE status_id IN (3) AND actived = 1 AND glass_id = products_glasses.id ORDER BY sort_n DESC LIMIT 1)))), (SELECT name FROM `groups` WHERE id = lists_to_cut.status_id)), '')
                        END), ' ', IF(products_glasses.new_id != 0, CONCAT('NEW ID: <b>',products_glasses.new_id,'</b>'), ''), ' ', CONCAT(IF(orders_product.picture IS NULL OR orders_product.picture = '','',CONCAT('<a class=\"f_img\" target=\"_blank\" style=\"color:blue;\"  href=\"',orders_product.picture,'\"><img style=\"width:35px;\" src=\"assets/img/main.png\"></a>')), IF(products_glasses.picture IS NULL OR products_glasses.picture = '','',CONCAT('<a class=\"f_img\" target=\"_blank\" style=\"color:blue;\"  href=\"',products_glasses.picture,'\"><img style=\"width:35px;\" src=\"assets/img/glass.png\"></a>')))) SEPARATOR ',<br>') AS glasses,
        CASE
...
function getPage($id, $res = ''){
    ...
    <fieldset class="fieldset">
        ...
        <div class="row">
            ...
            <div class="col-sm-12">------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------</div>
            <div class="col-sm-12">
                <label>პროდუქცია | მინაპაკეტი: <b id="minapaket_cc">0</b> | ლამექსი: <b id="lameqs_cc">0</b> | დუშკაბინა: <b id="dush_cc">0</b> | მინა: <b id="mina_cc">0</b></label><br><b>მინაპაკეტები:</b><br>';

                /* $db->setQuery("SELECT CONCAT('<span>',glass_options.name, '(',glass_manuf.name,') ', glass_type.name, ' ', glass_colors.name,IF(glasses_paths.id IS NULL,'',' (ნაწრთობი) '),' X ',COUNT(DISTINCT products_glasses.id),' ცალი</span>') AS glasses,


                                        ROUND(SUM((products_glasses.glass_width * products_glasses.glass_height))/1000000,2) AS kvdrt
                                ...
                                WHERE   products_glasses.actived = 1 AND products_glasses.order_id = '$id' AND glass_status.id IN (1,2,3,6)
                                ...
                                ORDER BY IF(glasses_paths.id IS NULL,0,1)");
                ...
                $db->setQuery("SELECT *,
                                ...
                                JOIN    glass_manuf ON glass_manuf.id = products_glasses.glass_manuf_id
                                LEFT JOIN		lists_to_cut ON lists_to_cut.glass_id = products_glasses.id AND lists_to_cut.actived = 1
                                WHERE   orders_product.order_id = '$id' AND orders_product.actived = 1 AND products_glasses.actived = 1 AND orders_product.product_id = 2
                                GROUP BY orders_product.id) AS gr_prods
                ...
                $db->setQuery("SELECT *,
                                ...
                                JOIN    glass_manuf ON glass_manuf.id = products_glasses.glass_manuf_id
                                LEFT JOIN		lists_to_cut ON lists_to_cut.glass_id = products_glasses.id AND lists_to_cut.actived = 1
                                WHERE   orders_product.order_id = '$id' AND orders_product.actived = 1 AND products_glasses.actived = 1 AND orders_product.product_id = 3
                                GROUP BY orders_product.id) AS gr_prods
                                GROUP BY gr_prods.glasses");
                $combined2 = $db->getResultArray();
                foreach($combined2['result'] AS $comb){
                ...
                $db->setQuery("SELECT CONCAT('<span>',glass_options.name, '(',glass_manuf.name,') ', glass_type.name, ' ', glass_colors.name,IF(glasses_paths.id IS NULL,'',' (ნაწრთობი) '),' X ',COUNT(DISTINCT products_glasses.id),' ცალი</span>') AS glasses,
                                ...
                                JOIN    glass_status ON glass_status.id = products_glasses.status_id
                                JOIN    glass_manuf ON glass_manuf.id = products_glasses.glass_manuf_id
                                JOIN    orders_product ON orders_product.id = products_glasses.order_product_id AND orders_product.product_id NOT IN (2,3)
                                LEFT JOIN	glasses_paths ON glasses_paths.glass_id = products_glasses.id AND glasses_paths.actived = 1 AND glasses_paths.path_group_id = 5
                                ...
                                GROUP BY products_glasses.glass_option_id, products_glasses.glass_color_id, products_glasses.glass_manuf_id, IF(glasses_paths.id IS NULL,0,1)
                                ORDER BY IF(glasses_paths.id IS NULL,0,1)");
                $combined3 = $db->getResultArray();

                foreach($combined3['result'] AS $comb){
                    $data .= $comb['glasses'].': <b>'.$comb['kvdrt'].'</b> კვ.მ<br>';
                }
...
function getFinishFew(){
    $data = '   <fieldset class="fieldset">
                    <legend>დაჯგუფებული მინები</legend>
                        <div class="row">
                            <div class="col-sm-12">
                                <div id="kalioni_group"></div>
                            </div>
